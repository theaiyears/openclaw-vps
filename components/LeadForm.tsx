...
export function LeadForm({ topic }: { topic: string }) {
  ...
  return <form id='lead-form' onSubmit={submit} style={{display:'flex',gap:8,marginTop:16}}><input required type='email' value={email} onChange={e=>setEmail(e.target.value)} placeholder='you@domain.com' style={{padding:10,borderRadius:8,border:'1px solid #334',flex:1}}/><button type='submit' style={{padding:'10px 14px',borderRadius:8,background:'#4f7cff',color:'white',border:0}}>Get the playbook</button><span style={{fontSize:12,opacity:.8}}>{status}</span></form>
}
